...
class PVPlantModel:
    ...
    inv_param: InitVar[pd.DataFrame]
    mod_param: InitVar[pd.DataFrame]
    temp_param: dict[str, dict[str, dict[str, Any]]] = field(
        default_factory=lambda: TEMPERATURE_MODEL_PARAMETERS["pvsyst"][
            "freestanding"
        ],
        repr=False,
    )
    ...
    def _create_pv_systems(
        ...
        if inv_df.is_empty():
            ...
            raise KeyError(msg)
        inv_dict = dict(
            inv_df.rows_by_key(key=["index"], named=True, unique=True)
        )
        ...
        mod_dict = dict(
            mod_df.rows_by_key(key=["index"], named=True, unique=True)
        )
    ...
    def _build_system_micro(
        ...
        """
        _LOGGER.debug(
            "Building microinverter system model for system %s", name
        )
        pv_systems = []

        # create a PVSystem for each microinverter
        for _, array in enumerate(arrays):
            n_modules = int(array["strings"]) * int(
                array["modules_per_string"]
            )
            mount = FixedMount(
    ...
    def _build_system_string(
        ...
        """
        _LOGGER.debug(
            "Building string inverter system model for system %s", name
        )
        pv_arrays = []
