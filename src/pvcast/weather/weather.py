...
class WeatherAPI(ABC):
    ...
    _weather_data: dict[str, Any] = field(
    ...
    def start_forecast(self) -> dt.datetime:
        ...
        return dt.datetime.now(tz.utc).replace(
    ...
    def get_weather(
        ...
        if (
            len(self._weather_data) > 0
            and self.dt_new_data < self.max_age
            and not live
        ):
            _LOGGER.debug(
                "Using cached weather data with age %s.", self.dt_new_data
            )
