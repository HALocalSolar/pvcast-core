...
class CommonWeatherTests:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                pd.concat([common_df.shift(-1).ffill()], axis=1),
                "Processed data contains duplicate",
            ),
            (
                common_df.sample(frac=1, random_state=1),  # Shuffle rows
                "Processed data is not sorted.",
            ),
            (
                common_df.reset_index().drop(
                    common_df.index[1]
                ),  # Drop row at index 1
...
class TestWeatherAPI(CommonWeatherTests):
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                pd.concat(
                    [common_df, pd.DataFrame({"temperature": [0, np.nan, 1]})],
                    axis=1,
                ),
            ...
            (
                pd.concat(
                    [
                        common_df,
                        pd.DataFrame({"invalid_column": [0] * len(common_df)}),
                    ],
                    axis=1,
                ),
            ...
            (
                pd.concat(
                    [common_df, pd.DataFrame({"temperature": [0, None, 1]})],
                    axis=1,
                ),
    ...
    def test_cloud_cover_to_irradiance_invalid_how(
        ...
        """Test cloud_cover_to_irradiance with invalid how argument."""
